...
def main():
	images_paths_numbers = [1, 31, 100, 150, 200] + [i * 10 for i in range(1, 20)]
	...
	for i in range(len(images_paths_numbers)):
		...
		if i > 4:
			...
			frame_image_array = generate_gray_scale_image_array(image_data)
			...
			frame_image_array = generate_rgb_image_array(image_data)
			frame_image_array = draw_line(frame_image_array, centroid_col, 0)
			scipy.misc.imsave('result_frames/frame_' + str(number) + '.jpg', frame_image_array)
